...
parent      =  extends Combine[Int] {
  def combine(x: Int, y: Int): Int = combine(x, _root_.scala.Predef.???)
} # -1,
parent span = <342..523>,
child       = def combine(x: Int, y: Int): Int = combine(x, _root_.scala.Predef.???) # -1,
child span  = [369..373..533]
...
object Combine:
    given Combine[Int] with
        def combine(x: Int, y: Int): Int =  combine(x, 
